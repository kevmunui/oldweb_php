...
include_once "global_setting.php";
 ...
 if(!isset($_POST['submit'])){
	...
	$query = "SELECT * FROM clients WHERE `id` = $_GET[id]"or die("Query one failed".mysql_error());
	$result = mysql_query($query)or die("Query two failed".mysql_error());
...
<div id="templatemo_wrapper">
    ...
    <div id="templatemo_main">
        ...
        <div class="clear"></div>
            ...
            <div id="comment_form">
           <h3>edit client's account</h3>
            
            <form action="update_client.php" method="POST">
                <span class="errormsg"><?php echo $msg ;?></span><br><br>

				<label>User's First Name (* required )</label>
				<input type='text' placeholder='first name' name='Fname' value='<?php echo @$person['fname'];?>' class="input_field">
				<label>User's second name (* required )</label>
				<input type='text' placeholder='second name' name='Sname' value='<?php echo @$person['sname'];?>' class="input_field">
				<label>User's phone number(* required )</label>
				<input type='number' placeholder='phone number' name='numb' value='<?php echo @$person['number'];?>' class="input_field">
				<label>User's email address (* required )</label>
				<input type='email' placeholder='email address' name='email' value='<?php echo @$person['email'];?>'class="input_field">
				<label>User's policy type (* required )</label>
				<input type='text' placeholder='policy type' name='policy' value='<?php echo @$person['policy'];?>' class="input_field">
				<label>User's initial premium (* required )</label>
				<input type='text' placeholder='premium' name='premiums' value='<?php echo @$person['premiums'];?>' class="input_field">
				<label>User's deposit (* required )</label>
				<input type='text' placeholder='deposit' name='deposit' value='<?php echo @$person['deposit'];?>'  class="input_field">
				...
				<br>
                <input type="submit" name="Submit" value="Change!" class="submit_btn" />
            </form>
