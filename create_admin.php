...
if(isset($_SESSION['user'])){
		...
		if(isset($_POST['user']) && isset($_POST['pass']) && isset($_POST['privilage']) && isset($_POST['email']))
		{
		
		
		$user= mysql_real_escape_string( $_POST['user']);
		$pass= mysql_real_escape_string( $_POST['pass']);
		$priv= mysql_real_escape_string($_POST['privilage']);
		$email= mysql_real_escape_string($_POST['email']);

		
		
		if((!empty($user))  && !empty($pass) && !empty($priv) && !empty($email)){
			...
			$query = "SELECT name FROM admin WHERE name = '$user'";
							...
							$msg="Oops $user already exists try another!";
							
							}
							else{
							
							$query = "INSERT INTO `admin`(name,password,priv,email) VALUES ( '$user','$pass','$priv','$email')";
							if($mysql_run = mysql_query($query)){
							
							$msg="You Have now registered $user !";
        ...
        <div class="clear"></div>
           ...
           <h3>Create new client's account</h3>
            ...
            <form action="create_admin.php" method="post">
				...
				<label>User's User Name (* required )</label>
				<input type='text' placeholder='name' name='user' class="input_field">
				<label>Password (* required )</label>
				<input type='password' placeholder='password' name='pass' class="input_field"><br>
				<label>New Administrator's Privilege (* required )</label>
				<input type='radio'  name='privilage' value='SU' >Manager
				<input type='radio'  name='privilage' value='NU' >Content creator<br><br><br>
