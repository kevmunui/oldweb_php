...
if(isset($_SESSION['user'])){
		...
		if(isset($_POST['Fname']) && isset($_POST['Sname']) && isset($_POST['user']) && isset($_POST['pass']) 
		&& isset($_POST['numb']) && isset($_POST['email']) && isset($_POST['policy']) &&  
		isset($_POST['premiums'])  && isset($_POST['deposit']) )
		{
		...
		if((!empty($fname)) && !empty($sname) && !empty($user) && !empty($pass) && !empty($numb) && !empty($pol) && !empty($prem) && !empty($dep) ){
		
			$query = "SELECT user FROM clients WHERE user = '$user' ";
							
							$result = mysql_query($query,$connection)or die("Query Failed:".mysql_error());
							
							if(mysql_num_rows($result) == 1 ){

							echo "<script language='javascript'>alert('OOPS! user code already exists! try another');</script>";
							$msg="'".$user."' already exists";
							}
							else{
							
							$query = "INSERT INTO `clients` VALUES ( '','".mysql_real_escape_string($fname)."','".mysql_real_escape_string($sname)."','".mysql_real_escape_string($user)."',
							'".mysql_real_escape_string($pass)."','".mysql_real_escape_string($numb)."','".mysql_real_escape_string($email)."','".mysql_real_escape_string($pol)."','".mysql_real_escape_string($prem)."','".mysql_real_escape_string($dep)."','','')";
							...
							$msg="we could not register you at this time";
							}
							}
...
<div id="templatemo_wrapper">
    ...
    <div id="templatemo_main">
        ...
        <div class="clear"></div>
            ...
            <div id="comment_form">
           <h3>Create new client's account</h3>
            
            <form action="new_client.php" method="post">
                <span class="errormsg"><?php echo $msg ;?></span><br><br>

				<label>User's First Name (* required )</label>
				<input type='text' placeholder='first name' name='Fname' class="input_field">
				<label>User's second name (* required )</label>
				<input type='text' placeholder='second name' name='Sname' class="input_field">
				<label>User's Number (* required )</label>
				<input type='number' placeholder='new usercode' name='user' class="input_field" >
				<label>User's Password (* required )</label>
				<input type='password' placeholder='password' name='pass' class="input_field" >
				<label>User's phone number(* required )</label>
				<input type='number' placeholder='phone number' name='numb' class="input_field">
				<label>User's email address (* required )</label>
				<input type='email' placeholder='email address' name='email' class="input_field">
				<label>User's policy type (* required )</label>
				<input type='text' placeholder='policy type' name='policy' class="input_field">
				<label>User's initial premium (* required )</label>
				<input type='text' placeholder='premium' name='premiums' class="input_field">
				<label>User's deposit (* required )</label>
				<input type='text' placeholder='deposit' name='deposit' class="input_field">
